...
def error(file_name):
    ...
    def tr_b_click(clicked): 
        ...
        elif args[1] == 'RTL-SDR not detected!':
            Thread(target=subprocess_launcher, args=['Scripts/send_data_online.py']).start()
        
    def tr_b_rel(released):
        try_btn['image'] = error_img_try_1_load
        sys.exit()

    try_btn = tk.Button(mainFrame, image=error_img_try_1_load, text='SET', width=66, height=20, borderwidth=0, relief=tk.SUNKEN, highlightthickness=0)
