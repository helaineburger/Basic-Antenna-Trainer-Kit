...
cent_freq_txt = tk.StringVar(value=cent_freq_val) 
...
samp_rate_txt = tk.StringVar(value=samp_rate_val)
...
def live_click(clicked):
    ...
    Thread(target=subprocess_launcher, args=['Scripts/anim_plot_online.pyw']).start()
...
def snap_click(clicked):
    ...
    Thread(target=subprocess_launcher, args=['Scripts/static_plot_online.pyw']).start()
